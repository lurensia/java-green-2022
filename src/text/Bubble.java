...
public class Bubble {
    public static void main(String[] args) {
        ...
        System.out.println("리스트에 들어갈 숫자를" + size + " 가지 쓰세요.");
        for (int i = 0; i < size; i++) {
            addNum = sc.nextInt();
            list.add(addNum);// 리스트에 숫자 추가.
            if (i == size - 1) {// 크기가 커질경우 자신이 어디까지 했는지 확인하기 위해
                System.out.println("\r마지막숫자가 입력되었습니다.\r");
            } else if (i % 5 == 0) {// 5단위로 자신이 적은 숫자의 갯수 확인.(이게 20쯤 되니 자기가 얼마나 썼는지 알 수가 없넹...)
                System.out.println( "\r" + i + 1 + "번째 숫자입니다.\r");
            } 
        }
        System.out.println("오름차순으로 정렬 하실 경우 1번을, \r내림차순의 경우 2번을 눌러주세요.\r");
        upDown = sc.nextInt();
        if (upDown == 1) {
            System.out.println("1. 오름차순으로 정렬합니다.\r");
            ...
            System.out.println("2.내림차순으로 정렬합니다.\r");
            for (x = 1; x < size; x++) {// 사이즈크기만큼 반복시작
                for (y = 0; y < size - x; y++) {
